...
export default class UIMenuDynamicListItem extends UIMenuItem {
    ...
    constructor(
        ...
        if (!this.isVariableFunction(selectionChangeHandler)) {
            alt.logError(
                `[UIMenuDynamicListItem] ${text} is not created with a valid selectionChangeHandler, needs to be function. Please see docs.`
            )
        }
        if (!this.isVariableFunction(selectedStartValueHandler)) {
            alt.logError(
                `[UIMenuDynamicListItem] ${text} is not created with a valid selectedStartValueHandler, needs to be function. Please see docs.`
            )
        }
    ...
    public set PreCaptionText(text) {
        ...
        this._currentOffset = Screen.GetTextWidth(
    ...
    public SelectionChangeHandlerPromise(
        item: UIMenuDynamicListItem,
