...
export default class NativeUI {
    ...
    constructor(title: string, subtitle: string, offset: Point, spriteLibrary?: string, spriteName?: string) {
        ...
        this._bannerSprite = new Sprite(
            this._spriteLibrary,
            this._spriteName,
            new Point(0 + this._offset.X, 0 + this._offset.Y),
            new Size(431, 107)
        )
        this._mainMenu.addItem(
            (this._titleResText = new ResText(
                title,
                new Point(215 + this._offset.X, 20 + this._offset.Y),
        ...
        if (subtitle !== '') {
            this._mainMenu.addItem(
                new ResRectangle(
                    new Point(0 + this._offset.X, 107 + this._offset.Y),
                    new Size(431, 37),
                    new Color(0, 0, 0, 255)
                )
            )
            this._mainMenu.addItem(
                (this._subtitleResText = new ResText(
                    subtitle,
        ...
        this._extraRectangleDown = new ResRectangle(
            new Point(
                0 + this._offset.X,
                144 + 18 + 38 * (this._maxItemsOnScreen + 1) + this._offset.Y - 37 + this._extraOffset
            ),
            new Size(431, 18),
            new Color(0, 0, 0, 200)
        )
        ...
        this._descriptionRectangle = new Sprite(
            'commonmenu',
            'gradient_bgd',
            new Point(this._offset.X, 127),
            new Size(431, 30)
        )
        this._descriptionText = new ResText(
            '',
            new Point(this._offset.X + 5, 125),
            0.35,
    ...
    public SetNoBannerType(): void {
        this._bannerSprite = null
        this._bannerRectangle = new ResRectangle(
            new Point(this._offset.X, this._offset.Y),
            new Size(431 + this.WidthOffset, 107),
            new Color(0, 0, 0, 0)
        )
    }
    ...
    public ProcessMouse() {
        ...
        if (Screen.IsMouseInBounds(new Point(0, 0), new Size(30, 1080)) && this._mouseEdgeEnabled) {
            ...
            Screen.IsMouseInBounds(new Point(Screen.ResolutionMaintainRatio.Width - 30.0, 0), new Size(30, 1080)) &&
            ...
            if (Screen.IsMouseInBounds(new Point(xpos, ypos), new Size(xsize, ysize))) {
                ...
                if (
                    ...
                    (this.MenuItems[i] instanceof UIMenuListItem ||
                        this.MenuItems[i] instanceof UIMenuAutoListItem ||
        ...
        const extraY =
            144 + 38 * (this._maxItemsOnScreen + 1) + this._offset.Y - 37 + this._extraOffset + this._safezoneOffset.Y
    ...
    public UpdateScaleform() {
        ...
        this._instructionalButtonsScaleform.callFunction(
            'SET_DATA_SLOT',
            0 as number,
            game.getControlInstructionalButtonsString(2, Control.PhoneSelect as number, false) as string,
            this.SelectTextLocalized as string
        )
        this._instructionalButtonsScaleform.callFunction(
            'SET_DATA_SLOT',
            1 as number,
            game.getControlInstructionalButtonsString(2, Control.PhoneCancel as number, false) as string,
            this.BackTextLocalized as string
        )
    ...
    private CleanUp(closeChildren: boolean = false) {
        ...
        this.MenuItems.filter((menuItem) => menuItem instanceof UIMenuDynamicListItem).forEach(
            (menuItem: UIMenuDynamicListItem) => {
